...
const closeConn = (doc, conn) => {
  if (DEBUG_LOGS) console.debug("closeConn", doc.name ?? doc.guid, doc.conns);
 ...
 * @param {WSSharedDoc} doc
 */
const subdocUpdateHandler = (update, origin, doc) => {
  const encoder = encoding.createEncoder();
...
class WSSharedDoc extends Y.Doc {
  ...
  constructor(name, isSubdoc) {
    ...
    if (isSubdoc) {
      this.on("update", subdocUpdateHandler);
    } else {
    ...
    switch (messageType) {
      case messageAuth: {
        ...
        if (conn.authenticated) {
          // inform client that hes authenticated
      ...
      case messageSubDocSync: {
        ...
        console.log("messageSubDocSync", subdocId);

        const subdoc = doc.loadSubdoc(subdocId, conn);
